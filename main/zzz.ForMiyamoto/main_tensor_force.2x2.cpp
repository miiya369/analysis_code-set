...
int main(int argc, char **argv)
{
   ...
   if (!use_JK_data) {
      printf("ERROR: This code can calculate only for Jack-Knife bin data.\n");
      return -1;
   }
   ...
   for (int conf=0; conf<analysis::Nconf; conf++)
   {
      ...
      for (int loop=0; loop<4; loop++) {
         ...
         for (int it=-1; it<=+1; it++) {
            Wave_org.set(channel, time_slice+it, conf);
...
#pragma omp parallel for
   ...
   /// Output coupled channel center & tensor potentials ///
...
static int set_args(int argc, char** argv)
{
   ...
   for (int loop=1; loop<argc; loop++) {
	 ...
	 else if (strcmp(argv[loop],"-mass00"    )==0)
	    mass[0] = atof(argv[loop+1]);
         else if (strcmp(argv[loop],"-mass01"    )==0)
	    mass[1] = atof(argv[loop+1]);
         else if (strcmp(argv[loop],"-mass10"    )==0)
	    mass[2] = atof(argv[loop+1]);
         else if (strcmp(argv[loop],"-mass11"    )==0)
	    mass[3] = atof(argv[loop+1]);
         else if (strcmp(argv[loop],"-src_t" )==0)
            analysis::set_data_list(N_T_SHIFT, "%s", argv[loop+1]);
	 else if (strcmp(argv[loop],"-zfac"    )==0)
	    Zfactor01 = atof(argv[loop+1]);
