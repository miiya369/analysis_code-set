...
static double  fit_range_min;
static double  fit_range_max;
...
int main(int argc, char **argv)
{
   ...
   /// Input the miyamoto-format fit data ///
...
//#pragma omp parallel for
   for (int conf=0; conf<analysis::Nconf; conf++) {
      ...
      chisq[conf] = (*fit)(conf).fit_data_NR(  cood, data, err, Ndata
...
{   
   ...
   for (int loop=1; loop<argc; loop++) {
      if (argv[loop][0] == '-'){
         ...
         else if (strcmp(argv[loop],"-r_min"   )==0)
            fit_range_min = atof(argv[loop+1]);
         else if (strcmp(argv[loop],"-r_max"   )==0)
            fit_range_max = atof(argv[loop+1]);
   ...
   FIT_FUNCTION tmp_fit_func; // Error check
   tmp_fit_func.set(fit_func_name.c_str());
   if (tmp_fit_func.Nparam != n_param) {
      printf("ERROR: #.param is different between initial and func_type.\n");
      return 1;
   }
   ...
   printf(" @ #. param    = %d\n @ param       = { ",n_param);
   for(int loop=0; loop<n_param; loop++) 
      if (param_initial[loop] > 0.0001)
         printf("%lf ",param_initial[loop]);
      else
         printf("%e ",param_initial[loop]);
   printf("}\n @ stop cond   = %lf\n\n",stp_cnd);
   fflush(stdout);
...
static int set_args_from_file(char* file_name)
{
   ...
   for (int loop=0; loop<count; loop++) {
      ...
      if (     strcmp(tmp_c1,"FIT_Stopping_condition")==0)
         ...
         fit_range_min = atof(tmp_c2);
      else if (strcmp(tmp_c1,"FIT_Fit_range_max"     )==0)
         fit_range_max = atof(tmp_c2);
