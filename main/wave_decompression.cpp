   ...
   for (int iconf=0; iconf<analysis::Nconf; iconf++)
   {
      ...
      for (int ich=0; ich<N_ch; ich++)
      {
         for (int Tslice=time_slice_min; Tslice<=time_slice_max; Tslice++)
         {
            ...
            NBSwave.set(channel_type[ich], Tslice, iconf, endian_flg, true);
            
            analysis::set_data_list(MAIN_PATH, "%s", opath);
            NBSwave.output(channel_type[ich], Tslice, iconf, endian_flg);
         }
      }
      time(&timer);
      printf(" @@@ conf=%d END || %s", iconf, ctime(&timer));
      fflush(stdout);
   }
...
static int set_args_from_file(char* file_name)
{
   ...
   for (int loop=0; loop<count; loop++)
   {
      ...
      else if (strcmp(tmp_c1,"DECO_Endian_convert"    )==0)
