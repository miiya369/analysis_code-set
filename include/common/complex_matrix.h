 ...
 * @file
 * @ingroup gauge
 * @brief   Header file for the complex matrix class (arbitrary rank) & SU3 class
...
//--------------------------------------------------------------------------
/**
 * @brief The class for the complex matrix (arbitrary dimension)
 */
//--------------------------------------------------------------------------
class cmatrix
{
...
//================== Constructor ==================//
   cmatrix() {
      mat = NULL;   // Default initialize is Dim = 2
      init(2);
   }
...
//================== Operator helper ==================//
   ...
   friend cmatrix operator+(cmatrix lhs, const double &rhs) {
      return lhs += std::complex<double>(rhs, 0.0);
   }
   friend cmatrix operator+(const std::complex<double> &lhs, cmatrix rhs) {
      return rhs += lhs;
   }
   friend cmatrix operator+(const double &lhs, cmatrix rhs) {
      return rhs += std::complex<double>(lhs, 0.0);
   }
   friend cmatrix operator-(cmatrix lhs, const cmatrix &rhs) {
      return lhs -= rhs;
   }
...
//================== Several function ==================//
   ...
   void set_zero() {
      for (size_t i=0; i<m_Ndim*m_Ndim; i++) (*this)(i) = COMP_ZERO;
   }
   void set_unit() {
      for (size_t i=0; i<m_Ndim; i++) for (size_t j=0; j<m_Ndim; j++)
         i==j ?
         (*this)(i,j) = std::complex<double>(1.0, 0.0) :
         (*this)(i,j) = COMP_ZERO;
   }
   cmatrix unit() {
      cmatrix ret(m_Ndim);
   ...
   std::complex<double> det() {
      if      (m_Ndim == 1)
         ...
         printf("WORNING: cmatrix.det() at Ndim > 3 is not implemented yet.\n");
   ...
   cmatrix inverce() {
      cmatrix tmp(m_Ndim);
      if      (m_Ndim == 1)
         tmp(0,0) = std::complex<double>(1.0, 0.0);
      else if (m_Ndim == 2) {
         tmp(0,0) =  (*this)(1,1);
         tmp(0,1) = -(*this)(0,1);
         tmp(1,0) = -(*this)(1,0);
