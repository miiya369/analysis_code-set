 ...
 * @file
 * @ingroup gauge
 * @brief   Header file for some functions of the vector and the matrix
...
//--------------------------------------------------------------------------
/**
 * @brief The functions for initialize by all-zero elements
 */
//--------------------------------------------------------------------------
void init_vec_zero(double *vec, const size_t Ndim)
{
   for (size_t i=0; i<Ndim; i++) vec[i] = 0.0;
}
void init_vec_zero(std::complex<double> *vec, const size_t Ndim)
{
   for (size_t i=0; i<Ndim; i++) vec[i] = COMP_ZERO;
}
void init_mat_zero(double *mat, const size_t Ndim)
{
   for (size_t i=0; i<Ndim*Ndim; i++) mat[i] = 0.0;
}
void init_mat_zero(std::complex<double> *mat, const size_t Ndim)
{
   for (size_t i=0; i<Ndim*Ndim; i++) mat[i] = COMP_ZERO;
}

//--------------------------------------------------------------------------
/**
 * @brief The functions for initialize by the unit-matrix (For matrix)
 */
//--------------------------------------------------------------------------
...
void init_mat_unit(std::complex<double> *mat, const size_t Ndim)
{
   for (size_t i=0; i<Ndim; i++) for (size_t j=0; j<Ndim; j++)
      mat[idx(i,j)] = (i==j ? std::complex<double>(1.0,0.0) : COMP_ZERO);
}

//--------------------------------------------------------------------------
/**
 * @brief The function for print the elements
 */
//--------------------------------------------------------------------------
void print_vec(const double *vec, const size_t Ndim)
{
   printf("[");
   for (size_t i=0; i<Ndim; i++) printf(" %1.1e",vec[i]);
   printf(" ]\n");
}
void print_vec(const std::complex<double> *vec, const size_t Ndim)
{
   printf("[");
   for (size_t i=0; i<Ndim; i++)
      printf(" (%1.1e,%1.1e)",vec[i].real(), vec[i].imag());
   printf(" ]\n");
}
...
std::complex<double> norm_vec(const std::complex<double> *vec, const size_t Ndim)
{
...
//--------------------------------------------------------------------------
/**
 * @brief The function for calculate the trace of the matrix
 */
//--------------------------------------------------------------------------
...
std::complex<double> trace_mat(const std::complex<double> *mat, const size_t Ndim)
{
   std::complex<double> ret = COMP_ZERO;
   for (size_t i=0; i<Ndim; i++) ret += mat[idx(i,i)];
...
//--------------------------------------------------------------------------
/**
 * @brief The function of multiplication X = Y x Z for v (vector) and M (matrix)
 */
//--------------------------------------------------------------------------
void mult_vv(  double *Xvec
             , const double *Yvec, const double *Zvec, const size_t Ndim )
{
   for (size_t i=0; i<Ndim; i++) Xvec[i] = Yvec[i] * Zvec[i];
}
void mult_vv(  std::complex<double> *Xvec
             , const std::complex<double> *Yvec, const std::complex<double> *Zvec
             , const size_t Ndim )
{
   for (size_t i=0; i<Ndim; i++) Xvec[i] = Yvec[i] * Zvec[i];
}
void mult_vM(  double *Xvec
             , const double *Yvec, const double *Zmat, const size_t Ndim )
{
   for (size_t j=0; j<Ndim; j++) {
      Xvec[j] = 0.0;
      for (size_t i=0; i<Ndim; i++)
         Xvec[j] += Yvec[i] * Zmat[idx(i,j)];
   }
}
void mult_vM(  std::complex<double> *Xvec
             , const std::complex<double> *Yvec, const std::complex<double> *Zmat
             , const size_t Ndim )
{
   for (size_t j=0; j<Ndim; j++) {
      Xvec[j] = COMP_ZERO;
...
void mult_Mv(  double *Xvec
             , const double *Ymat, const double *Zvec, const size_t Ndim )
{
   for (size_t i=0; i<Ndim; i++) {
      Xvec[i] = 0.0;
      for (size_t j=0; j<Ndim; j++)
         Xvec[i] += Ymat[idx(i,j)] * Zvec[j];
   }
}
void mult_Mv(  std::complex<double> *Xvec
             , const std::complex<double> *Ymat, const std::complex<double> *Zvec
             , const size_t Ndim )
{
   for (size_t i=0; i<Ndim; i++) {
      Xvec[i] = COMP_ZERO;
...
void mult_MM(  double *Xmat
             , const double *Ymat, const double *Zmat, const size_t Ndim )
{
#ifdef _OPENMP
   ...
   for (   size_t i=0; i<Ndim; i++)
      for (size_t j=0; j<Ndim; j++) {
         ...
         for (size_t k=0; k<Ndim; k++)
            Xmat[idx(i,j)] += Ymat[idx(i,k)] * Zmat[idx(k,j)];
   }
}
void mult_MM(  std::complex<double> *Xmat
             , const std::complex<double> *Ymat, const std::complex<double> *Zmat
             , const size_t Ndim )
{
#ifdef _OPENMP
#pragma omp parallel for
 ...
 * @brief (return the un-difference)
 */
//--------------------------------------------------------------------------
...
std::complex<double> check_mat_symmetry(  const std::complex<double> *mat
                                        , const size_t Ndim )
{
   std::complex<double> ret = 0.0;
   for (size_t i=0; i<Ndim; i++) for (size_t j=0; j<Ndim; j++)
                           ...
                           , const int Nvalue , const double stop_cnd )
{
   ...
   for (size_t i=1; i<Ndim-1; i++)   // find the maximum eigen-value
   {
